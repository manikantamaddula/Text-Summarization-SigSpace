...
object Kmeans_NV {
  ...
  def main(args: Array[String]) {
    ...
    println("top TFIDF words for all classes done")
    ...
    val vec2=vec_scaled.select("vector2","word")
    val vec_3 =  vec2.filter(vec_scaled("word").isin(topwordterms.toList:_*))
    val vec3=vec_3.rdd
    ...
    val x3=vec_3.select("vector2").rdd.map{ f=>f.mkString(" ")}
